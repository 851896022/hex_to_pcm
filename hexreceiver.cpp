...
{
    timer->stop();
    //int aa=dataCache.count();
    while(dataCache.count()>51)
    {

        if((dataCache[0]==(char)0xFB && dataCache[1]==(char)0))//如果包头符合音频包格式
        {
           //qDebug()<<"包头符合FB00";
           if(dataCache.count()>10)
           {
               //qDebug()<<"长度大于10";
               if(dataCache[6]==(char)0xFF && dataCache[7]==(char)0xFB)
               {
                   //qDebug()<<"符合FFFB";
        ...
        else if((dataCache[0]==(char)0xFC && dataCache[1]==(char)0))//如果包头符合幅度包格式
        {
            if(dataCache.count()>=54)
            {
                if((dataCache[52]==(char)0xFB && dataCache[53]==(char)0))
                {
...
void hexReceiver::decode(QByteArray ch)
{
    ...
    if(realsize>51)
    {
        ...
        if((ch[0]==(char)0xFB&&ch[1]==(char)0)&&realsize==594)
        {
//            static int lastNo;
//            unsigned char thisNo=ch[3];
//            if(lastNo == thisNo-1)
//            {

//            }
//            else if(lastNo == 255 && thisNo == 0)
//            {
...
//            lastNo=thisNo;
        ...
        else if((ch[0]==(char)0xFC&&ch[1]==(char)0)&&realsize==52)
        {
